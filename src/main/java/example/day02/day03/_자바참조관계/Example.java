...
public class Example {
    public static void main(String[] args) {
        ...
        // 참조 -> 참조 -> 참조 -> .(도트연산자)이용하여 가능
        category.set카테고리번호(1);
        category.set카테고리명("공지사항");
        // 3. 변수가 참조하는 인스턴스에는 2개의 멤버변수를 참조할 수 있음
        // category 변수 참조 횟수 : 3개
        ...
        Reply reply = Reply.builder()
                .댓글번호(1)
                .댓글내용("댓글입니다") // 1번 댓글에 1번 게시물 인스턴스 대입
                .board(board)
                .build();
        // reply 변수 참조 횟수 :
        ...
        // [ 단방향 ]
        // 7. reply 객체에서 board 객체와 category 객체를 알 수 있나? [ O ]
        // 7-1. category 객체에서 board 객체와 reply 객체를 알 수 있나? [ X ]
        System.out.println();

        // [ 양방향 ] 데이터베이스 미사용 / 가바 가능
        // 8. reply 객체에서 board 객체와 category 객체를 알 수 있나? [ O ]
        // 8-1. category 객체에서 board 객체와 reply 객체를 알 수 있나? [ O ]
        ...
        // * StackOverflowError (JVM[Stack]의 메모리가 넘침)
        // -> toString() 메소드때문에 순환 참조됨 (무한루프)
        // category(toString 포함) <----> boardList(toString 포함)
            // 해결방안 : toString() 사용하지 않기. 또는 사용하되 양방향일 경우 한쪽 방향에 @ToString .Exclude 사용
    }
}
