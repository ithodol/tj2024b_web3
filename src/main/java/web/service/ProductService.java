...
public class ProductService {
    ...
    public boolean deleteProduct(long pno, int loginMno){
        ...
        if(productEntity.getMemberEntity().getMno() != loginMno){
            // 만약 제품 등록한 회원의 번호와 현재 로그인된 회원번호가 일치하지 않으면 null
            return false;
        }
        // 4. 서버에 저장(업로드)된 이미지들 삭제
        List<ImgEntity> imgEntityList = productEntity.getImgEntityList();
        for(ImgEntity imgEntity : imgEntityList){
            boolean result = fileUtil.fileDelete(imgEntity.getIname()); // web2에서 작성한 파일삭제 메소드 참고
            if(result == false){
                throw new RuntimeException("파일 삭제 실패"); // 트랜잭션 롤백
            }
        }
        // 5. 이미지 모두 삭제했으면 DB 삭제
            // cascade = CascadeType.All 제품이 삭제되면 이미지 레코드도 같이 삭제됨
        ...
        if( newFile != null && !newFile.isEmpty() ){ // 새로운 이미지가 존재하면
            for( MultipartFile file : newFile ){
                ...
                ImgEntity imgEntity = ImgEntity.builder() // 이미지 엔티티 생성 , , 자바객체 <--영속X--> DB테이블레코드
                        ...
                        .build();
                imgRepository.save( imgEntity ); // 이미지 엔티티 저장(영속) , 자바객체 <--영속O--> DB테이블레코드
            }
        }
        return true; // 6. 끝
    } // class end
